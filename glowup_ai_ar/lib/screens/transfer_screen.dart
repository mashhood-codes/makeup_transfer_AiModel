...
class _TransferScreenState extends State<TransferScreen> {
  ...
  Future<void> _loadStyles() async {
    try {
      print('📥 Loading styles from API...');
      final styles = await _api.getStyles();
      print('✅ Styles loaded: ${styles.length} styles');
      for (var style in styles) {
        print('   - ${style['id']}: ${style['thumbnail']}');
      }
      setState(() => _apiStyles = styles);
    } catch (e) {
      print('❌ Error loading styles: $e');
      // Set empty list so UI doesn't crash
      setState(() => _apiStyles = []);
    }
  }
  ...
  Future<Uint8List> _loadStyleImage(String thumbnailPath) async {
    try {
      final url = '$_serverUrl$thumbnailPath';
      print('🖼️ Fetching style image: $url');
  ...
  Future<void> _checkServer() async {
    ...
    if (!connected) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Server not found. Check IP settings.')),
        );
      }
    }
  }
                  ...
                  child: ClipRRect(
                    ...
                    child: FutureBuilder<Uint8List>(
                      ...
                      builder: (context, snapshot) {
                        ...
                        print('✅ Thumbnail loaded: ${style['id']}');
                        return Image.memory(snapshot.data!, fit: BoxFit.cover);
                      },
                    ),
                  ),
  ...
  Widget _buildResult() {
    return Column(
      ...
      children: [
        const Text(
          'Result',
          style: TextStyle(fontSize: 16, fontWeight: FontWeight.w600, color: Color(0xFF2D1B4E)),
        ),
        const SizedBox(height: 12),
        Container(
          decoration: BoxDecoration(
          ...
          child: ClipRRect(
            ...
            child: Image.network(
              ...
              fit: BoxFit.cover,
              errorBuilder: (context, error, stackTrace) {
                print('❌ Result image load error: $error');
                return Container(
                  color: Colors.grey[300],
                  child: Column(
                    mainAxisAlignment: MainAxisAlignment.center,
              ...
              loadingBuilder: (context, child, loadingProgress) {
                ...
                print('⏳ Loading result image... ${loadingProgress.cumulativeBytesLoaded}/${loadingProgress.expectedTotalBytes}');
                return Center(
